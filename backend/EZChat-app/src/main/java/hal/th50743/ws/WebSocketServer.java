 ...
 * 
 * <p>本类负责处理所有 WebSocket 连接的生命周期管理、消息路由和状态同步。
 * 采用单例模式设计，所有连接共享静态资源，确保线程安全和资源高效利用。</p>
 * 
 * <h3>核心职责：</h3>
 * <ul>
...
public class WebSocketServer {
     ...
     * Service 依赖注入方法
     * 
     * <p>由于 WebSocket 端点不是由 Spring 直接管理，无法使用常规的 @Autowired 注解。
     * 通过此静态 setter 方法实现依赖注入，确保服务单例被正确初始化。</p>
     * 
     * <p><b>注入时机</b>：Spring 容器启动时自动调用</p>
     * <p><b>线程安全</b>：静态变量，所有连接共享同一服务实例</p>
     * 
     ...
     * WebSocket 连接建立处理方法
     * 
     * <p>当客户端发起 WebSocket 连接请求时触发此方法。主要职责包括：</p>
     * <ol>
     ...
     * </ol>
     * 
     * <p><b>重连机制</b>：检查是否有 pending 的离线任务，如果有且能取消，则视为网络波动重连，
     * 不广播上线状态，避免频繁的状态切换通知。</p>
     * 
     * <p><b>安全考虑</b>：Token 过期会返回 4001 状态码，认证失败返回 4002 状态码，
     * 前端可根据状态码进行相应的错误处理。</p>
     * 
     ...
     * WebSocket 消息接收处理方法
     * 
     * <p>当客户端通过 WebSocket 发送消息时触发此方法。处理流程：</p>
     * <ol>
     ...
     * </ul>
     * 
     * <p><b>确认机制</b>：每条消息处理成功后，会给发送者发送 ACK 确认，
     * 前端可根据 ACK 更新消息发送状态。</p>
     * 
     ...
     * WebSocket 连接关闭处理方法
     * 
     * <p>当客户端断开 WebSocket 连接时触发此方法。采用防抖机制设计，
     * 避免网络波动导致的误判，提升用户体验。</p>
     * 
     * <h3>业务逻辑流程：</h3>
     * <ol>
     ...
     * </ul>
     * 
     * <p><b>线程安全考虑</b>：使用 final 局部变量捕获实例状态，确保延迟任务访问的数据一致性。</p>
     * 
     ...
     * </ul>
     * 
     * <p><b>性能考虑</b>：循环遍历目标用户列表，对于大规模广播可考虑优化为批量操作。</p>
     * 
     ...
     * </ul>
     * 
     * <p><b>线程安全</b>：使用 synchronized 确保同一连接的消息顺序发送。</p>
     * <p><b>连接检查</b>：发送前验证会话是否仍然有效。</p>
     * 
     ...
     * 安全关闭 WebSocket 会话方法
     * 
     * <p>用于在认证失败或异常情况下优雅地关闭连接，并向客户端返回特定的关闭原因。</p>
     * 
     * <p><b>使用场景</b>：</p>
     * <ul>
     ...
     * 消息有效性验证方法
     * 
     * <p>验证接收到的消息是否符合业务规则，防止非法或格式错误的消息进入处理流程。</p>
     * 
     * <h3>验证规则：</h3>
     * <ol>
     ...
     * </ol>
     * 
     * <p><b>安全考虑</b>：严格验证发送者身份，防止用户冒充他人发送消息。</p>
     * 
     ...
     * 获取在线用户列表（只读视图）
     * 
     * <p>提供当前所有在线用户的会话映射表，用于监控、调试或其他业务需求。</p>
     * 
     * <p><b>线程安全</b>：返回的是 ConcurrentHashMap 的引用，调用者应注意并发访问安全。</p>
     * <p><b>只读建议</b>：除非必要，调用者不应修改返回的映射表内容。</p>
     * 
     ...
     * 
     * <p>全局消息广播接口，允许从任何地方向指定用户列表发送消息。
     * 常用于系统通知、状态同步等需要主动推送的场景。</p>
     * 
     * <h3>使用场景：</h3>
     * <ul>
     *   <li>系统公告推送</li>
