...
CREATE TABLE `chat_sequences` (
                                  ...
                                  `current_seq`  BIGINT UNSIGNED DEFAULT 0 NOT NULL COMMENT '当前最大序列号',
                                  `update_time`  DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL ON UPDATE CURRENT_TIMESTAMP,
                                  PRIMARY KEY (`chat_id`)
...
CREATE TABLE `messages` (
                            ...
                            `seq_id`      INT UNSIGNED NOT NULL COMMENT '群内消息序号 (1, 2, 3...)',
                            `sender_id`   INT UNSIGNED NOT NULL,
                            `type`        TINYINT DEFAULT 0 NOT NULL COMMENT '0:Text, 1:Img, 2:Mixed, 10:System...',
                            `text`        LONGTEXT NULL,
                            `asset_ids`   VARCHAR(4096) NULL COMMENT '图片ID列表 JSON数组 [1, 2]',
                            `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL,
                            `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL ON UPDATE CURRENT_TIMESTAMP,
    -- 核心索引：确保同一群内序号唯一且递增，同时优化按序号查询
                            UNIQUE INDEX `uk_chat_seq` (`chat_id`, `seq_id`),
...
CREATE TABLE `operation_logs` (
                                  ...
                                  `module`         VARCHAR(50) NOT NULL COMMENT '功能模块 (如: Chat, User, Message)',
                                  `type`           VARCHAR(50) NOT NULL COMMENT '操作类型 (如: CREATE, UPDATE, DELETE)',
                                  `content`        VARCHAR(2048) DEFAULT NULL COMMENT '详细描述 (方法名 + 参数摘要)',
                                  `ip_address`     VARCHAR(50) DEFAULT NULL COMMENT '操作IP',
                                  `status`         TINYINT DEFAULT 1 COMMENT '1=成功, 0=失败',
                                  `execution_time` BIGINT DEFAULT 0 COMMENT '方法执行耗时(毫秒)',
                                  `request_path`   VARCHAR(255) DEFAULT NULL COMMENT '请求路径 (如: /api/chat/create)',
                                  ...
                                  `target_id`      VARCHAR(100) DEFAULT NULL COMMENT '目标资源ID (如chatId, messageId)',
                                  `error_message`  VARCHAR(1024) DEFAULT NULL COMMENT '错误信息 (失败时记录)',
                                  `create_time`    DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
BEGIN
    ...
    -- =============================================
    -- Step 6: 生成消息 (Text + Image 引用 + 序列号逻辑)
    -- =============================================
    SET i = 1;
    WHILE i <= 20 DO -- 遍历 20 个群
    SET j = 1;

    -- 初始化该群的序列号 (虽然默认是0，但显式插入更安全，或者最后统一更新)
    INSERT INTO chat_sequences (chat_id, current_seq) VALUES (i, 0);

    WHILE j <= 50 DO -- 每个群生成 50 条消息
    SELECT user_id INTO rand_user_id FROM chat_members WHERE chat_id = i ORDER BY RAND() LIMIT 1;

    -- 使用循环变量 j 作为 seq_id，保证严格递增 (1...50)
    SET v_current_seq = j;
    ...
    IF msg_type = 0 THEN
        INSERT INTO messages (sender_id, chat_id, seq_id, type, text, create_time)
        VALUES (rand_user_id, i, v_current_seq, 0, 'Hello, this is a test message.', DATE_SUB(NOW(), INTERVAL (50 - j) MINUTE));
    ELSE
        -- 随机选取一个种子对象 (ID 1-30) 作为消息图片
        SET v_seed_id = FLOOR(1 + RAND() * 30);

        INSERT INTO messages (sender_id, chat_id, seq_id, type, asset_ids, create_time)
        VALUES (rand_user_id, i, v_current_seq, 1, CONCAT('[', v_seed_id, ']'), DATE_SUB(NOW(), INTERVAL (50 - j) MINUTE));
    END IF;
    ...
    -- 更新该群的全局序列号为最大值 (50)
    UPDATE chat_sequences SET current_seq = 50 WHERE chat_id = i;
