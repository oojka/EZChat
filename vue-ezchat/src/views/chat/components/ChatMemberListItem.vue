...
type ChatMemberView = Omit<User, 'isOnline'> & { isOnline: boolean }
interface Props { member: ChatMemberView; isMe: boolean }
...
<template>
  <div class="member-item" :class="{ 'is-me': isMe, 'is-offline': !member.isOnline }">
    <div class="avatar-wrapper">
      <el-badge is-dot :offset="[-2, 34]" :type="isMe ? wsDisplayState.type : (member.isOnline ? 'success' : 'info')" class="status-dot">
        <el-avatar :size="40" :src="member.avatar.objectThumbUrl" class="member-avatar">
...
.me-tag { font-size: 9px; padding: 1px 6px; background: var(--primary); color: #fff; border-radius: 100px; font-weight: 900; letter-spacing: 0.5px; flex-shrink: 0; box-shadow: 0 2px 4px rgba(59, 130, 246, 0.2); }
</style>
